...
#define Si4709_IRQ MSM_GPIO_TO_INT(29)
//#define Si4709_IRQ 1
...
#if 0
	if (gpio_is_valid(GPIO_FM_RST_N)) {
		if (gpio_request(GPIO_FM_RST_N, S3C_GPIO_LAVEL(GPIO_FM_RST_N))) 
			printk(KERN_ERR "Failed to request GPIO_FM_RST_N!\n");
		gpio_direction_output(GPIO_FM_RST_N, GPIO_LEVEL_LOW);
	}
	s3c_gpio_setpull(GPIO_FM_INT, S3C_GPIO_PULL_DOWN); 
#endif
    
	/****Resetting the device****/
	gpio_set_value(GPIO_FM_RST_N, GPIO_LEVEL_LOW);	
	gpio_set_value(GPIO_FM_RST_N, GPIO_LEVEL_HIGH);	
