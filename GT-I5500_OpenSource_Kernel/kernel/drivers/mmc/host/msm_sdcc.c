...
do_resume_work(struct work_struct *work)
{
	...
	desc = irq_to_desc(host->plat->status_irq);

	if (mmc) {
		mmc_resume_host(mmc);
/* do_resume_work is not called in the booting time */
#if IRQ_SUSPEND_ENABLE
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	if (!mmc) {
		printk(KERN_ERR "%s: mmc_alloc_host() failed\n:", __func__);
		ret = -ENOMEM;
		goto out;
	}
...
static int msmsdcc_auto_suspend(struct mmc_host *host, int suspend)
{
	...
	int ret;
	struct mmc_host *mmc;
	struct msmsdcc_host *msm_host;
	struct irq_desc *desc;

	pdev = container_of(host->parent, struct platform_device, dev);
	mmc = mmc_get_drvdata(pdev);
	msm_host = mmc_priv(mmc);
	desc = irq_to_desc(msm_host->plat->status_irq);

	if (suspend)
	{
		ret = msmsdcc_suspend(pdev, PMSG_AUTO_SUSPEND);
		if(desc->status & IRQ_DISABLED)
		enable_irq(msm_host->plat->status_irq);
	}
	else
	{
		ret = msmsdcc_resume(pdev);
	}
