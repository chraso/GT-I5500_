...
static void report_key(struct gpio_kp *kp, int key_index, int out, int in)
{
	...
	if (pressed != test_bit(keycode, kp->input_dev->key)) {
		if (keycode == KEY_RESERVED) {
			if ((mi->flags & GPIOKPF_PRINT_UNMAPPED_KEYS) && (europa_hw_revision < 4))
